...
public class LoginController {
    ...
    public String doLogin(@ModelAttribute("user") User user, Model model, @CookieValue(value = "setUser",defaultValue = "") String setUser, HttpServletResponse response, HttpServletRequest request){
        if (user.getEmail().equals("ad@gmail.com")&&user.getPassword().equals("12345")){
            ...
            model.addAttribute("message","Dang nhap thanh cong.Xin chao!");
        }else {
            user.setEmail("");
            Cookie cookie =new Cookie("setUser",setUser);
            model.addAttribute("cookieValue",cookie);
            model.addAttribute("message","Loi.Xin thu lai!");
        }
        return "/login";
    }
